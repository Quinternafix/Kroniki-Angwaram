...
function resolveCharacterId(item, characters) {
    ...
    match = list.find(
        c => String(c.name || "").toLowerCase() === rawLower
    );
    if (match) {
        return match.id;
    }

    // 4. Slug of name or id equals slug of item
    const itemSlug = slugify(raw);
    match = list.find(
        c =>
            slugify(c.id) === itemSlug ||
            slugify(c.name) === itemSlug
    );
    if (match) {
        return match.id;
    }

    // Fallback: slugified label (may 404 for unknown NPCs)
    return itemSlug;
}
 ...
 * Klucz = wartość zapisana w JSON (najczęściej po polsku)
 */
                ...
                list.length
                    ? `
                        <ul>
                            ...
                            ${list.map(item => {
                                // Obsługa nowego formatu (obiekt) i starego (string)
                                ...
                                return `
                                    <li>
                                        ${
                                            relationLabel
                                                ? `<span class="relation">${escapeHtml(relationLabel)}</span> `
                                                : ""
                                        }
